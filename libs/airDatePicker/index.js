!(function (e, t) {
	...
	return (function () {
		...
		class _ {
			...
			_getClassName() {
				...
				let s = new Date(),
					...
					m = c('air-datepicker-cell', '-'.concat(this.singleType, '-'), { '-current-': u(s, this.date, this.type), '-min-date-': r && u(r, this.date, this.type), '-max-date-': h && u(h, this.date, this.type) }),
					g = '';
				switch (this.type) {
					case i.days:
			...
			_handleRangeStatus() {
				let { rangeDateFrom: e, rangeDateTo: t } = this.dp,
					i = c({ '-in-range-': e && t && ((s = this.date), (a = e), (n = t), p(s, a) && m(s, n)), '-range-from-': e && u(this.date, e, this.type), '-range-to-': t && u(this.date, t, this.type) });
				var s, a, n;
		...
		class T {
			...
			_buildBaseHtml() {
				(this.$el = n({ className: 'air-datepicker-body -'.concat(this.type, '-'), innerHtml: $[this.type] })), (this.$names = a('.air-datepicker-body--day-names', this.$el)), (this.$cells = a('.air-datepicker-body--cells', this.$el));
			}
			_getDayNamesHtml() {
		...
		class V {
			...
			_buildBaseHtml() {
				let { prevHtml: e, nextHtml: t } = this.opts;
				this.$el.innerHTML =
					'<div class="air-datepicker-nav--action" data-action="prev">'.concat(e, '</div>') + '<div class="air-datepicker-nav--title"></div>' + '<div class="air-datepicker-nav--action" data-action="next">'.concat(t, '</div>');
			}
			get isNavIsFunction() {
		...
		class j {
					...
					I(this, 'update', (e) => {
						...
						this._createMinMaxDates(),
							...
							!t.isMobile && h
								...
								: t.isMobile && !h && (this._removeMobileAttributes(), this.visible && (B.classList.remove('-active-'), 'function' != typeof this.opts.position && this.setPosition())),
			...
			replaceDate(e, t) {
				...
				a < 0 || u(this.selectedDates[a], t, this.currentView) || ((this.selectedDates[a] = t), this.trigger(i.eventChangeSelectedDate, { action: i.actionSelectDate, date: t, updateTime: !0 }), this._updateLastSelectedDate(t));
			}
			clear() {
			...
			_handleRangeOnFocus() {
				1 === this.selectedDates.length &&
					(p(this.selectedDates[0], this.focusDate) ? ((this.rangeDateTo = this.selectedDates[0]), (this.rangeDateFrom = this.focusDate)) : ((this.rangeDateTo = this.focusDate), (this.rangeDateFrom = this.selectedDates[0])));
			}
			getCell(e) {
